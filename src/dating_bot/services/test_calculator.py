"""
Сервис для расчета результатов психологического теста
"""
...
def get_questions_by_category(category: str) -> List[Dict]:
    """Возвращает все вопросы категории"""
...
def calculate_category_scores(answers_dict: Dict[int, int]) -> Dict[str, float]:
    """
    Рассчитывает средние баллы по категориям из ответов
    
    Args:
        answers_dict: {question_id: answer} где answer от 1 до 5
        
    Returns:
        Dict с баллами по категориям: {"communication_emotions": 3.5, ...}
    """
    category_scores = {}
    category_counts = {}
...
def format_results_for_display(category_scores: Dict[str, float]) -> str:
    """
    Форматирует результаты для красивого отображения пользователю
    """
...
def calculate_test_completion_percentage(answers_count: int) -> float:
    """Рассчитывает процент завершения теста"""
    total = get_total_questions()
    return round((answers_count / total) * 100, 1)


def get_min_required_answers() -> int:
    """Минимальное количество ответов для валидного теста"""
    return 20  


def is_test_complete(answers_count: int) -> bool:
    """Проверяет, достаточно ли ответов для завершения теста"""
